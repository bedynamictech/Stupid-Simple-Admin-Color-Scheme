...
function ssacs_add_menu() {
    add_menu_page(
        ...
        'manage_options',
        'stupidsimple',
        'ssacs_settings_page_content', // ← fixed callback
        'dashicons-hammer',
        99
    );
                ...
                <select name="<?php echo esc_attr(SSACS_OPTION); ?>" id="ssacs_default_scheme">
                    ...
                    foreach ($schemes as $scheme) {
                        printf(
                            ...
                            esc_attr($scheme),
                            selected($scheme, $current_scheme, false),
                            esc_html(ucfirst($scheme))
                        );
                    }
                    ?>
                </select>
...
// Register the settings with a sanitize callback.
add_action('admin_init', 'ssacs_register_settings');
function ssacs_register_settings() {
    register_setting(
        'ssacs_settings_group',
        SSACS_OPTION,
        'ssacs_update_all_users_scheme'
    );
}

// Callback: sanitize & update all users
...
function ssacs_set_default_scheme_for_new_user($user_id) {
    ...
    if ($default_scheme) {
        update_user_meta(
            $user_id,
            'admin_color',
            sanitize_text_field($default_scheme)
        );
    }
}
...
function ssacs_enqueue_styles() {
    ...
    if ($default_scheme) {
        wp_enqueue_style(
            'ssacs-admin-styles',
            plugins_url('ssacs-styles.css', __FILE__),
            array(),
            '1.0'
        );
        $inline_css = "#wpwrap { --wp-admin-theme-color: var(--wp-admin-color-" 
                    . esc_attr($default_scheme) . "); }";
        wp_add_inline_style('ssacs-admin-styles', $inline_css);
    }
}

// Hide the admin color scheme selector for non‑admins.
...
// Optional: Admin notice if no default has been set.
add_action('admin_notices', 'ssacs_admin_notice');
function ssacs_admin_notice() {
    if ( current_user_can('manage_options') 
      && ! get_option(SSACS_OPTION) 
    ) {
        ...
        printf(
            ...
            sprintf(
                ...
                __('The Stupid Simple Admin Color Scheme plugin is active, but you haven\'t set a default scheme yet. Please visit the <a href="%s">Admin Color Scheme settings page</a> to configure it.', 'ssacs'),
                esc_url(admin_url('admin.php?page=admin-color-scheme'))
            )
        );
        echo '</div>';
    }
